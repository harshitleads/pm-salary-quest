...
const SalaryTierCard = ({ tier }: { tier: typeof salaryTiers[number] }) => {
  ...
  return (
    <button
      ...
      className={`${tier.gradient} group relative w-full overflow-hidden rounded-2xl p-6 lg:p-4 text-left shadow-lg transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl active:scale-[0.98]`}
    >
      ...
      <div className="relative z-10 flex flex-col gap-3">
        ...
        <p className="text-hero-title lg:text-lg text-white drop-shadow-md">{tier.salary}</p>
        <p className="text-base lg:text-sm font-medium text-white/80">{tier.companies}</p>
        <span className="mt-2 inline-flex items-center gap-1 text-quiz-option lg:text-xs font-semibold text-white underline-offset-4 group-hover:underline">
